...
//    input[31:0] offset
   ...
   // debug signals
output wire[31:0] debug_control_reg,
output wire[31:0] debug_status_reg,
output wire[31:0] debug_buffer_reg0,
output wire[31:0] debug_buffer_reg1,
...
assign debug_control_reg = control_reg;
assign debug_status_reg = status_reg;
assign debug_buffer_reg0 = buffer_reg_0;
assign debug_buffer_reg1 = buffer_reg_1;
...
wire[31:0] bits_to_send = (bytes_to_send  * 32'd8 );
wire[31:0] bits_to_receive = (bytes_to_receive  * 32'd8);
...
wire spi_clk_rise_phase  = (clock_cycles_counter < (clock_period/2));
wire spi_clk_fall_phase  = (clock_cycles_counter >= (clock_period/2));
wire spi_period_tick = (clock_cycles_counter == (clock_period) - 1);

wire spi_fall_tick = (clock_cycles_counter == clock_period/2);
wire spi_rise_tick = (clock_cycles_counter == 32'b0);

           assign SPI_CLK = ((state == TRANSMIT || state == RECEIVE) && spi_clk_rise_phase);
...
assign debug_spi_state = state;
assign debug_bytes_to_send = bytes_to_send;
assign debug_bits_to_send = bits_to_send;
assign debug_bits_sent = bits_counter;
...
always @(*) begin
    ...
    //  clock_cycles_counter_next = clock_cycles_counter ;
         clock_cycles_counter_next = 32'b0 ;
...
//status_reg_next[1] = 1'b0;
      ...
      TRANSMIT:begin
         ...
         end  else if(spi_rise_tick)begin
            MOSI = buffer_reg_0[0];

         end else if(spi_fall_tick)begin
      ...
      RECEIVE : begin
       // write control_reg[25] back to 0;
        ...
        status_reg_next[1] = 1'b1;
         ...
         end else if(spi_clk_rise_phase)begin
            buffer_reg_1_next = {buffer_reg_1[30:0] , MISO};

         end else if(spi_clk_fall_phase)begin
